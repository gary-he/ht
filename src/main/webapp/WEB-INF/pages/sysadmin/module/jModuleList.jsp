...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'">
		...
		<td>
			<c:if test="${module.state==1}">主菜单</c:if>
			<c:if test="${module.state==2}">左侧菜单</c:if>
			<c:if test="${module.state==3}">按钮</c:if>
		</td>
		<td>${module.orderNo }</td>
		<td>${module.remark }</td>
		<td>
			<c:if test="${module.state==1}"><a href="stop?moduleId=${module.moduleId}"><font color="green">启用</font></a></c:if>
			<c:if test="${module.state==0}"><a href="start?moduleId=${module.moduleId}"><font color="red">停用</font></a></c:if>
		</td>
	</tr>
	</c:forEach>
